...
gap_train <- gap_train %>%
  mutate(
    log_direvent = log(no_direvent + 0.00000001),
...
gap_test <- gap_test %>%
  mutate(
    log_direvent = log(no_direvent + 0.0000001),
    #         log_viewcart = log(viewcart + 0.0000001),
    log_wishlist = log(wishlist + 0.0000001)
  )
...
auc_nn <- Metrics::auc(
  actual = ifelse(eval_dat[training == 0, ]$buyer == "yes", 1, 0),
  predicted = eval_dat[training == 0, ]$nn
)

## auc in val
### test <- predict(nn_refit, pred_data = gap_val)[, 16]
auc_nn_val <- Metrics::auc(
  actual = ifelse(gap_val$buyer == "yes", 1, 0),
  predicted = predict(nn_result, pred_data = gap_val)$Prediction
)

## confusion matrix
confusionMatrix(
  data = ifelse(eval_dat[training == 0, ]$nn > 0.5, "yes", "no"),
...
confusionMatrix(
  data = ifelse(predict(logit, newdata = gap_test, type = "prob")[, 2] > 0.5, "yes", "no"),
